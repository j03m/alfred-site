...
export default function TableTabs({ holdings, predictions }: TableTabsProps) {
  const [activeTab, setActiveTab] = useState<'holdings' | 'predictions'>('holdings');

  useEffect(() => {
    const savedTab = localStorage.getItem('alfred-table-tab');
    if (savedTab === 'holdings' || savedTab === 'predictions') {
      setActiveTab(savedTab);
    }
  }, []);

  const handleTabChange = (tab: 'holdings' | 'predictions') => {
    setActiveTab(tab);
